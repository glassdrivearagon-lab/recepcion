// GLASSDRIVE MVP - COMPLETO CON BÚSQUEDA Y PDF REAL
...
class GlassDriveMVP {
    ...
    updateSteps() {
        ...
        // CRÍTICO: Actualizar botones del footer
    ...
    generatePDFForExpedient(expedient, isNew = false) {
        try {
            ...
            if (!expedient.fotos.length && isNew) {
            ...
            setTimeout(() => {
                ...
                doc.text('GLASSDRIVE', 105, 20, { align: 'center' });

                doc.setFontSize(12);
                doc.text('Informe de Recepción de Vehículo', 105, 30, { align: 'center' });

                // INFORMACIÓN DEL CENTRO
                const centerName = expedient.centro || this.getTallerName(this.currentTaller);
                doc.setFont('helvetica', 'normal');
                doc.setFontSize(10);
                doc.text(`Centro: ${centerName}`, 20, 45);
                doc.text(`Fecha: ${new Date().toLocaleDateString()}`, 20, 52);
                doc.text(`Hora: ${new Date().toLocaleTimeString()}`, 20, 59);
                ...
                doc.setFontSize(14);
                doc.setTextColor(26, 77, 114);
                doc.text('INFORMACIÓN DEL VEHÍCULO', 20, 80);

                doc.setFont('helvetica', 'normal');
                doc.setFontSize(11);
                doc.setTextColor(0, 0, 0);
                doc.text(`Matrícula: ${expedient.matricula || 'No detectada'}`, 20, 92);
                doc.text(`Cliente: ${expedient.cliente || 'Cliente MVP Demo'}`, 20, 102);
                doc.text(`Vehículo: ${expedient.vehiculo || 'Vehículo Demo'}`, 20, 112);

                // DOCUMENTACIÓN
                doc.setFont('helvetica', 'bold');
                doc.setFontSize(14);
                doc.setTextColor(26, 77, 114);
                doc.text('DOCUMENTACIÓN', 20, 130);
                ...
                doc.text(`Fotografías: ${fotosCount} archivos`, 20, 142);
                doc.text(`Ficha técnica: ${fichaPresente}`, 20, 152);
                doc.text(`Póliza de seguro: ${polizaPresente}`, 20, 162);

                // CALIBRACIÓN
                doc.setFont('helvetica', 'bold');
                doc.setFontSize(14);
                doc.setTextColor(26, 77, 114);
                doc.text('CALIBRACIÓN', 20, 180);
                ...
                if (expedient.requiereCalibracion) {
                    doc.text('Requiere calibración: SÍ', 20, 192);
                    doc.text('Estado: Registrado como pendiente', 20, 202);
                    doc.text(`Fecha registro calibración: ${new Date().toLocaleDateString()}`, 20, 212);
                } else {
                    doc.text('Requiere calibración: NO', 20, 192);
                    doc.text('Estado: No aplicable', 20, 202);
                }
                ...
                if (expedient.firma) {
                    doc.text('Firma del cliente: Capturada digitalmente', 20, 242);
                    doc.text(`Fecha firma: ${new Date().toLocaleDateString()}`, 20, 252);
                } else {
                    doc.text('Firma del cliente: No capturada', 20, 242);
                }
                ...
                // DESCARGAR PDF
                const fileName = `GlassDrive-${expedient.matricula || 'MVP'}-${centerName}-${new Date().toISOString().split('T')[0]}.pdf`;
                doc.save(fileName);
